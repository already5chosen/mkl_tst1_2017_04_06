...
struct noncblas_sgemm_prm_t {
  ...
  fp_vector_t   cc[SIMD_ELEM_PEC_COL_MJ*A_WORDS_PER_ITER];
 ...
 int                   nRows)    // 0 < nRows    <= k_step
{
  ...
  for (m = 0; m < pPrm->M-A_WORDS_PER_ITER+1;
    ...
    for (int ci = 0; ci < A_WORDS_PER_ITER; ++ci) {
      for (int wi = 0; wi < n_bIters*B_WORDS_PER_ITER; ++wi)
        _mm_prefetch((char*)(&CPrefetch[wi*SIMD_FACTOR]), _MM_HINT_T0);
      CPrefetch += ldc;
    }

    fp_vector_t* cc = pPrm->cc;
    for (int b_it = 0; b_it < n_bIters; cc += B_WORDS_PER_ITER, ++b_it) {
      ...
      cc[SIMD_ELEM_PEC_COL_MJ*0+0] = acc00; cc[SIMD_ELEM_PEC_COL_MJ*0+1] = acc10;
      cc[SIMD_ELEM_PEC_COL_MJ*1+0] = acc01; cc[SIMD_ELEM_PEC_COL_MJ*1+1] = acc11;
      cc[SIMD_ELEM_PEC_COL_MJ*2+0] = acc02; cc[SIMD_ELEM_PEC_COL_MJ*2+1] = acc12;
      cc[SIMD_ELEM_PEC_COL_MJ*3+0] = acc03; cc[SIMD_ELEM_PEC_COL_MJ*3+1] = acc13;
      cc[SIMD_ELEM_PEC_COL_MJ*4+0] = acc04; cc[SIMD_ELEM_PEC_COL_MJ*4+1] = acc14;
    }
  ...
  scalar_t *C, int ldc)
{
  ...
  // printf("nMaj=%d, nRemMin=%d, nRemMaj=%d remW_n=%d n_step=%d\n", nMaj, nRemMin, nRemMaj, remW_n, n_step);
