...
static void CopyAndMultiplyAC(gemm_prm_t* pPrm, unsigned nCol, unsigned nRow, const scalar_t *src, int isC)
{
  ...
  if (!isC) {
    ...
    ldsrc = pPrm->lda;
    mask  = pPrm->mask_k[0];
    mult  = MM_BROADCAST_Sx(&pPrm->alpha);
  } else {
    dst   = pPrm->cc;
    ldsrc = pPrm->ldc;
    mask  = pPrm->mask_n[0];
    mult  = MM_BROADCAST_Sx(&pPrm->beta);
  }
  do {
    const scalar_t *srcrow = src;
    for (int c = 0; c < nFullCol; ++c) {
      *dst = MM_MUL_Px(MM_LOADU_Px(srcrow), mult);
      dst    += 1;
      srcrow += SIMD_FACTOR;
    }
    if (nPartialCol) {
      //*dst = MM_MUL_Px(MM_LOADU_Px(srcrow), mult);
      *dst = MM_MUL_Px(MM_MASKLOADU_Px(srcrow, mask), mult);
      dst += 1;
    }
    src += ldsrc;
  } while (--nRow);
...
#if 0
static void CopyBC(gemm_prm_t* pPrm, unsigned nCol, unsigned nRow, const scalar_t *src, int isC)
{
  unsigned nFullCol    = nCol / SIMD_FACTOR;
  unsigned nPartialCol = nCol % SIMD_FACTOR;
  fp_vector_t* dst;
...
static void CopyBackC(gemm_prm_t* pPrm, unsigned nCol, unsigned nRow, scalar_t *dst)
{
  unsigned nFullCol    = nCol / SIMD_FACTOR;
  unsigned nPartialCol = nCol % SIMD_FACTOR;
  ...
  do {
    scalar_t *dstrow = dst;
    for (int c = 0; c < nFullCol; ++c) {
      MM_STOREU_Px(dstrow, *src);
      src    += 1;
      dstrow += SIMD_FACTOR;
    }
    if (nPartialCol) {
      MM_MASKSTOREU_Px(dstrow, mask, *src);
      src += 1;
    }
    dst += lddst;
  } while (--nRow);
...
// printf("m_step=%u k_step=%u n_step=%u\n", m_step, k_step, n_step);
  ...
  const int aa_sz = ((k_step-1)/SIMD_FACTOR + 1)*m_step;
  const int bb_sz = ((n_step-1)/SIMD_FACTOR + 1)*k_step;
  const int cc_sz = ((n_step-1)/SIMD_FACTOR + 1)*m_step;
  const int workBufSz = (aa_sz + bb_sz + cc_sz)*sizeof(fp_vector_t);
  ...
  for (int n = 0; n < N; n += n_step) {
    ...
    for (int m = 0; m < M; m += m_step) {
      ...
      if (prm.beta == 0.0f) {
        ...
        CopyAndMultiplyAC(&prm, delta_n, delta_m, Csrcdst, 1);
      }

      const scalar_t *Arow = &A[lda*m];
      prm.aa = prm.cc + nw*delta_m;
      for (int k = 0; k < K; k += k_step) {
        int delta_k = K-k < k_step ? K-k : k_step;
        unsigned kw = (unsigned)(delta_k-1)/SIMD_FACTOR+1;
        prm.ldaa = kw * SIMD_FACTOR;
        CopyAndMultiplyAC(&prm, k+prm.ldaa > K ? delta_k : prm.ldaa, delta_m, &Arow[k], 0);
        prm.bb = prm.aa + kw*delta_m;
        //CopyBC(&prm, delta_n, delta_k, &B[ldb*k+n], 0);
        CopyB(&prm, delta_n, delta_k, &B[ldb*k+n]);
        noncblas_sgemm_core(&prm, nw, delta_m, delta_k);
      }

      CopyBackC(&prm, delta_n, delta_m, Csrcdst);
    }
  }
  free(workBufAlloc);
}