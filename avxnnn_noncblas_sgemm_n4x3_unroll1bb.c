  ...
  for (m = pPrm->M - A_WORDS_PER_ITER + 1; m > 0;
    ...
    for (int b_it = 0; b_it <= b_itLast; Crow += B_WORDS_PER_ITER*SIMD_FACTOR, ++b_it) {
      ...
      if (!prepare_bb) {
        ...
        bb[1] = b1;
        bb[2] = b2;
        b0 = MM_LOADU_Px(&B[3*SIMD_FACTOR]);
        b1 = MM_LOADU_Px(&B[4*SIMD_FACTOR]);
        b2 = MM_LOADU_Px(&B[5*SIMD_FACTOR]);
        bb[ldbb+0] = b0;
        bb[ldbb+1] = b1;
        bb[ldbb+2] = b2;
        B  += ldb;
        ...
        do {
          ...
          _mm_prefetch((char*)(&B[ldb+0*SIMD_FACTOR])+0,   _MM_HINT_T0);
          _mm_prefetch((char*)(&B[ldb+0*SIMD_FACTOR])+96,  _MM_HINT_T0);
          _mm_prefetch((char*)(&B[ldb+0*SIMD_FACTOR])+191, _MM_HINT_T0);
          // _mm_prefetch((char*)(&B[6*SIMD_FACTOR])+0,   _MM_HINT_T1);
          // _mm_prefetch((char*)(&B[6*SIMD_FACTOR])+96,  _MM_HINT_T1);
          // _mm_prefetch((char*)(&B[6*SIMD_FACTOR])+191, _MM_HINT_T1);
