enum {
 ...
 A_WORDS_PER_ITER     = 5,
 B_WORDS_PER_ITER     = 2,
 B_ITERS_PER_CORE_MAX = 2,
 SIMD_ELEM_PEC_COL_MJ = B_WORDS_PER_ITER*B_ITERS_PER_CORE_MAX,
...
struct noncblas_sgemm_prm_t {
  ...
  // fp_vector_t   bb[SIMD_ELEM_PEC_COL_MJ*bb_nRows];
  // fp_vector4_t  aa[(aa_nCols*m_step_max)/4];
  fp_vector_t*  bb; // [SIMD_ELEM_PEC_COL_MJ*bb_nRows];
  fp_vector4_t* aa; // [(aa_nCols*m_step_max)/4];
};
...
static void fma256_noncblas_sgemm_core_mj(
 ...
 scalar_t*                   C,
 int                         n_bIters, // 0 < n_bIters <= B_ITERS_PER_CORE_MAX
...
int gl_m_step_nom = m_step_nom;
int gl_m_step_max = m_step_max;
  ...
  scalar_t *C, int ldc)
{
  ...
  const int m_step_nom = gl_m_step_nom;
  const int m_step_max = gl_m_step_max;
  const int k_step     = gl_k_step;
  const int bb_sz = SIMD_ELEM_PEC_COL_MJ*k_step;
  const int aa_sz = (m_step_max*k_step-1)/SIMD_FACTOR + 1;
  ...
  for (int k = 0; k < K; k += k_step) {
    ...
    for (int m = 0; m < M; m += prm.M) {
      ...
      for (n = 0; n < nMj; n += n_step) {
        ...
        CopyAndTransposeMj(&prm, &B[k*ldb + n], ldb, B_ITERS_PER_CORE_MAX, delta_k);
        fma256_noncblas_sgemm_core_mj(&prm, &Crow[n], B_ITERS_PER_CORE_MAX, delta_k);
      }
      if (nwRemMn > 0) {
        if (nwRemMj == 0)
