...
// major core - inner loop processes 2 SIMD columns of B x 3 rows of A
 ...
 int                         nRows)    // 0 < nRows    <= k_step
{
  ...
  for (m = 0; m < pPrm->M-A_WORDS_PER_ITER+1;
    ...
    for (int b_it = 0; b_it <= b_itLast; Crow += B_WORDS_PER_ITER*SIMD_FACTOR, ++b_it) {
      ...
      fp_vector_t acc00a = MM_SETZERO_Px();
      fp_vector_t acc10a = MM_SETZERO_Px();
      fp_vector_t acc01a = MM_SETZERO_Px();
      fp_vector_t acc11a = MM_SETZERO_Px();
      fp_vector_t acc02a = MM_SETZERO_Px();
      fp_vector_t acc12a = MM_SETZERO_Px();
      fp_vector_t acc00b = MM_SETZERO_Px();
      fp_vector_t acc10b = MM_SETZERO_Px();
      fp_vector_t acc01b = MM_SETZERO_Px();
      fp_vector_t acc11b = MM_SETZERO_Px();
      fp_vector_t acc02b = MM_SETZERO_Px();
      fp_vector_t acc12b = MM_SETZERO_Px();
      ...
      for (int k = 0; k < kSteps; ++k) {
        ...
        acc00a = MM_FMADD(a, b0, acc00a);
        acc10a = MM_FMADD(a, b1, acc10a);

        a = MM_BROADCAST_Sx(&ARow[4*1+0]);
        acc01a = MM_FMADD(a, b0, acc01a);
        acc11a = MM_FMADD(a, b1, acc11a);

        a = MM_BROADCAST_Sx(&ARow[4*2+0]);
        acc02a = MM_FMADD(a, b0, acc02a);
        acc12a = MM_FMADD(a, b1, acc12a);
        ...
        acc00b = MM_FMADD(a, b0, acc00b);
        acc10b = MM_FMADD(a, b1, acc10b);

        a = MM_BROADCAST_Sx(&ARow[4*1+1]);
        acc01b = MM_FMADD(a, b0, acc01b);
        acc11b = MM_FMADD(a, b1, acc11b);

        a = MM_BROADCAST_Sx(&ARow[4*2+1]);
        acc02b = MM_FMADD(a, b0, acc02b);
        acc12b = MM_FMADD(a, b1, acc12b);
      ...
      fp_vector_t acc00 = MM_ADD_Px(acc00a, acc00b);
      fp_vector_t acc10 = MM_ADD_Px(acc10a, acc10b);
      fp_vector_t acc01 = MM_ADD_Px(acc01a, acc01b);
      fp_vector_t acc11 = MM_ADD_Px(acc11a, acc11b);
      fp_vector_t acc02 = MM_ADD_Px(acc02a, acc02b);
      fp_vector_t acc12 = MM_ADD_Px(acc12a, acc12b);
      ...
      if (b_it != pPrm->masked_b_it) {
        if (pPrm->c_option == C_OPTION_UPDATE) {
          ...
          UPDATE_CCOL(CCol, acc02, acc12);  Prefetch2lines(CCol);
...
// minor core - inner loop processes 1 SIMD columns of B x 3 rows of A
 ...
 int                         nRows)    // 0 < nRows <= k_step
{
  ...
  for (m = 0; m < pPrm->M - A_WORDS_PER_ITER + 1; m += A_WORDS_PER_ITER) {
    ...
    for (int k = 0; k < kSteps; ++k) {
      ...
      b = Bcol[0];
      acc00a = MM_FMADD(MM_BROADCAST_Sx(&ARow[4 * 0 + 0]), b, acc00a);
      acc01a = MM_FMADD(MM_BROADCAST_Sx(&ARow[4 * 1 + 0]), b, acc01a);
      acc02a = MM_FMADD(MM_BROADCAST_Sx(&ARow[4 * 2 + 0]), b, acc02a);

      b = Bcol[1];
      acc10b = MM_FMADD(MM_BROADCAST_Sx(&ARow[4 * 0 + 1]), b, acc10b);
      acc11b = MM_FMADD(MM_BROADCAST_Sx(&ARow[4 * 1 + 1]), b, acc11b);
      acc12b = MM_FMADD(MM_BROADCAST_Sx(&ARow[4 * 2 + 1]), b, acc12b);

      b = Bcol[2];
      acc00a = MM_FMADD(MM_BROADCAST_Sx(&ARow[4 * 0 + 2]), b, acc00a);
      acc01a = MM_FMADD(MM_BROADCAST_Sx(&ARow[4 * 1 + 2]), b, acc01a);
      acc02a = MM_FMADD(MM_BROADCAST_Sx(&ARow[4 * 2 + 2]), b, acc02a);

      b = Bcol[3];
      acc10b = MM_FMADD(MM_BROADCAST_Sx(&ARow[4 * 0 + 3]), b, acc10b);
      acc11b = MM_FMADD(MM_BROADCAST_Sx(&ARow[4 * 1 + 3]), b, acc11b);
      acc12b = MM_FMADD(MM_BROADCAST_Sx(&ARow[4 * 2 + 3]), b, acc12b);
