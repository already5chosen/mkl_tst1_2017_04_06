  ...
  for (m = pPrm->M - A_WORDS_PER_ITER + 1; m > 0;
    ...
    for (int b_it = 0; b_it <= b_itLast; Crow += B_WORDS_PER_ITER*SIMD_FACTOR, ++b_it) {
      ...
      #define Prefetch2lines(x) \
        _mm_prefetch((char*)(x) + preftechDistance+0,                       _MM_HINT_T0); \
        _mm_prefetch((char*)(x) + preftechDistance+sizeof(fp_vector_t)/2*3, _MM_HINT_T0); \
        _mm_prefetch((char*)(x) + preftechDistance+sizeof(fp_vector_t)*3-1, _MM_HINT_T0);
      ...
      if (b_it != pPrm->masked_b_it) {
        if (pPrm->c_option == C_OPTION_UPDATE) {
          ...
          UPDATE_CCOL(CCol, acc00, acc10, acc20);  Prefetch2lines(CCol); CCol += ldc;
          UPDATE_CCOL(CCol, acc01, acc11, acc21);  Prefetch2lines(CCol); CCol += ldc;
          UPDATE_CCOL(CCol, acc02, acc12, acc22);  Prefetch2lines(CCol); CCol += ldc;
          UPDATE_CCOL(CCol, acc03, acc13, acc23);  Prefetch2lines(CCol);
