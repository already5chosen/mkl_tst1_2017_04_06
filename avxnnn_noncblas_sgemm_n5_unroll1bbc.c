 ...
 int                         nRows)    // 0 < nRows    <= k_step
{
  ...
  if (m > 0) {
    ...
    if (m == 4) {
      for (int b_it = 0; b_it <= b_itLast; C += B_WORDS_PER_ITER*SIMD_FACTOR, ++b_it) {
        const scalar_t* ARow = A;
        fp_vector_t acc00, acc10;
        fp_vector_t acc01, acc11;
        fp_vector_t acc02, acc12;
      ...
      for (int b_it = 0; b_it <= b_itLast; C += B_WORDS_PER_ITER*SIMD_FACTOR, ++b_it) {
        ...
        if (kSteps != 0) {
          ...
          acc00 = MM_ADD_Px(acc00, acc00b);
          acc10 = MM_ADD_Px(acc10, acc10b);
          acc01 = MM_ADD_Px(acc01, acc01b);
          acc11 = MM_ADD_Px(acc11, acc11b);
        }
        if (kRem) {
          b0 = Bcol[0];
          b1 = Bcol[1];
      ...
      for (int b_it = 0; b_it <= b_itLast; C += B_WORDS_PER_ITER*SIMD_FACTOR, ++b_it) {
        ...
        if (kSteps != 0) {
          fp_vector_t acc00b = MM_SETZERO_Px();
          fp_vector_t acc10b = MM_SETZERO_Px();
          fp_vector_t acc00c = MM_SETZERO_Px();
          fp_vector_t acc10c = MM_SETZERO_Px();
          fp_vector_t acc00d = MM_SETZERO_Px();
          fp_vector_t acc10d = MM_SETZERO_Px();
          int k = kSteps;
          do {
            b0 = Bcol[0];
            b1 = Bcol[1];
            ...
            a = MM_BROADCAST_Sx(&ARow[0]);
            acc00c = MM_FMADD(a, b0, acc00c);
            acc10c = MM_FMADD(a, b1, acc10c);
            ...
            a = MM_BROADCAST_Sx(&ARow[0]);
            acc00d = MM_FMADD(a, b0, acc00d);
            acc10d = MM_FMADD(a, b1, acc10d);
          ...
          acc00  = MM_ADD_Px(acc00, acc00b);
          acc10  = MM_ADD_Px(acc10, acc10b);
          acc00c = MM_ADD_Px(acc00c, acc00d);
          acc10c = MM_ADD_Px(acc10c, acc10d);
          acc00  = MM_ADD_Px(acc00, acc00c);
          acc10  = MM_ADD_Px(acc10, acc10c);
        }
        if (kRem) {
          int k = kRem;
          do {
 ...
 int                         nRows)    // 0 < nRows <= k_step
{
  ...
  for (m = pPrm->M - A_WORDS_PER_ITER + 1; m > 0; m -= A_WORDS_PER_ITER) {
  ...
  m += A_WORDS_PER_ITER - 1;
  // handle remaining rows of a, interleaved
  for (int mi = m; mi > 0; ARow += 1, --mi) {
    const fp_vector_t* Bcol = pPrm->bb;
    fp_vector_t acc00 = MM_SETZERO_Px();
    fp_vector_t acc01 = MM_SETZERO_Px();
    fp_vector_t acc02 = MM_SETZERO_Px();
    ...
    const scalar_t* Ar = ARow;
    for (int k = 0; k < kSteps; ++k) {
      acc00 = MM_FMADD(MM_BROADCAST_Sx(Ar),  Bcol[0], acc00); Ar += m;
      acc01 = MM_FMADD(MM_BROADCAST_Sx(Ar), Bcol[1], acc01); Ar += m;
      acc02 = MM_FMADD(MM_BROADCAST_Sx(Ar), Bcol[2], acc02); Ar += m;
      acc03 = MM_FMADD(MM_BROADCAST_Sx(Ar), Bcol[3], acc03); Ar += m;
      Bcol += 4;
    }
    if (kRem != 0) {
      acc00 = MM_FMADD(MM_BROADCAST_Sx(Ar), Bcol[0], acc00);
      if (kRem != 1) {
        Ar += m;
        acc01 = MM_FMADD(MM_BROADCAST_Sx(Ar), Bcol[1], acc01);
        if (kRem != 2) {
          Ar += m;
          acc02 = MM_FMADD(MM_BROADCAST_Sx(Ar), Bcol[2], acc02);
        }
      }
    }
...
static void CopyAndInterleaveA(noncblas_sgemm_prm_t* pPrm, const scalar_t *A, int n_cols)
{
  ...
  if (r != 0) {
    if (r == 4) {
      for (int k = 0; k < n_cols; A += 1, dst += r, ++k) {
        scalar_t a0 = A[0];
        scalar_t a1 = A[lda1];
        scalar_t a2 = A[lda2];
        scalar_t a3 = A[lda3];
