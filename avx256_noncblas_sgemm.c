 ...
 float*                      C)
{
  ...
  for (m = 0; m < pPrm->M-1; A += lda*2, C += ldc*2, m += 2) {
    ...
    for (int n = 0; n < SIMD_ELEM_PEC_COL; n += COLS_PER_LOOP) {
      ...
      __m256 a1 = _mm256_broadcast_ss(&ARow[lda]);
      __m256 b;
        b = Bcol[0];
      ...
      _mm_prefetch((char*)&Crow0[SIMD_FACTOR*0], _MM_HINT_T0);
      _mm_prefetch((char*)&Crow1[SIMD_FACTOR*0], _MM_HINT_T0);
      _mm_prefetch((char*)&Crow0[SIMD_FACTOR*1], _MM_HINT_T0);
      _mm_prefetch((char*)&Crow1[SIMD_FACTOR*1], _MM_HINT_T0);
      _mm_prefetch((char*)&Crow0[SIMD_FACTOR*2], _MM_HINT_T0);
      _mm_prefetch((char*)&Crow1[SIMD_FACTOR*2], _MM_HINT_T0);

      for (int k = 1; k < bb_nRows; ARow += 2, Bcol += SIMD_ELEM_PEC_COL, k += 2) {
        a0 = _mm256_broadcast_ss(&ARow[0]);
        a1 = _mm256_broadcast_ss(&ARow[lda]);
        ...
        Bcol += SIMD_ELEM_PEC_COL;
        a0 = _mm256_broadcast_ss(&ARow[1]);
        a1 = _mm256_broadcast_ss(&ARow[lda+1]);
