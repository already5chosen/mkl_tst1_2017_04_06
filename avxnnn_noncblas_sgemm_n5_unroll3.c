...
{
  int ldc = pPrm->ldc;
  int kSteps = (unsigned)(nRows-1) / 3;
  int kRem   = (unsigned)(nRows-1) % 3;
  ...
  scalar_t *C, int ldc)
{
  ...
  const int K_STEP_NOM = (K_STEP/3)*3 + 1;
  ...
  k_step = k_Nsteps < 2 ? K : ((K-1)/(k_Nsteps*3) + 1) * 3 + 1;
#endif
