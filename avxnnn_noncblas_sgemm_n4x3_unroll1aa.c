 ...
 int                         nRows)    // 7 < nRows    <= k_step
{
  ...
  ptrdiff_t lda1 = pPrm->lda;
  ptrdiff_t lda2 = lda1 + lda1;
  ptrdiff_t lda3 = lda2 + lda1;
  ptrdiff_t lda4 = lda3 + lda1;
  scalar_t*       aa = pPrm->aa;
  ...
  for (m = pPrm->M - A_WORDS_PER_ITER + 1; m > 0;
    ...
    int prepare_aa = pPrm->prepare_aa;
    for (int b_it = 0; b_it <= b_itLast; Crow += B_WORDS_PER_ITER*SIMD_FACTOR, ++b_it) {
      fp_vector_t* Bcol = &pPrm->bb[ldbb*b_it];
      scalar_t* CPrefetch = Crow;
      fp_vector_t a;

      fp_vector_t acc00, acc10, acc20;
      fp_vector_t acc01, acc11, acc21;
      fp_vector_t acc02, acc12, acc22;
      fp_vector_t acc03, acc13, acc23;

      if (!prepare_aa) {
        const scalar_t* ARow = aa;
        ...
        acc10 = MM_MUL_Px(a, b1);
        acc20 = MM_MUL_Px(a, b2);
        _mm_store_ss(&aaRow[0], _mm256_castps256_ps128(a));
        Prefetch3words(CPrefetch); CPrefetch += ldc;
        ...
        acc11 = MM_MUL_Px(a, b1);
        acc21 = MM_MUL_Px(a, b2);
        _mm_store_ss(&aaRow[1], _mm256_castps256_ps128(a));
        Prefetch3words(CPrefetch); CPrefetch += ldc;
        ...
        acc12 = MM_MUL_Px(a, b1);
        acc22 = MM_MUL_Px(a, b2);
        _mm_store_ss(&aaRow[2], _mm256_castps256_ps128(a));
        Prefetch3words(CPrefetch); CPrefetch += ldc;
        ...
        acc13 = MM_MUL_Px(a, b1);
        acc23 = MM_MUL_Px(a, b2);
        _mm_store_ss(&aaRow[3], _mm256_castps256_ps128(a));
        Prefetch3words(CPrefetch); CPrefetch += ldc;
      ...
      if (b_it != pPrm->masked_b_it) {
        if (pPrm->c_option == C_OPTION_UPDATE) {
          ...
          UPDATE_CCOL(CCol, acc02, acc12, acc22); CCol += ldc;
          UPDATE_CCOL(CCol, acc03, acc13, acc23);
  ...
  for (; m > 0; aa += nRows, A += lda1, C += ldc, --m) {
    scalar_t* Crow = C;
    int prepare_aa = pPrm->prepare_aa;
    for (int b_it = 0; b_it <= b_itLast; Crow += B_WORDS_PER_ITER*SIMD_FACTOR, ++b_it) {
      ...
      if (!prepare_aa) {
        ...
        ARow += 1;
        acc00 = MM_MUL_Px(a, Bcol[0]);
        acc10 = MM_MUL_Px(a, Bcol[1]);
        acc20 = MM_MUL_Px(a, Bcol[2]);
        Bcol += B_WORDS_PER_ITER;
        ...
        scalar_t* aaRow = aa;
        _mm_store_ss(aaRow, _mm256_castps256_ps128(a));
        ...
        do {
          ...
          _mm_storeu_ps(aaRow, _mm_loadu_ps(ARow));
          ARow  += 4;
          aaRow += 4;
        } while (--k);
...
static void CopyAndInterleaveA(noncblas_sgemm_prm_t* pPrm, const scalar_t *A, int n_cols)
{
  ...
  for (r = pPrm->M+1-A_WORDS_PER_ITER; r > 0; A += lda4, r -= A_WORDS_PER_ITER) {
    const scalar_t *src = A;
    for (int k = 0; k < n_cols; src += 1, dst += 4, ++k) {
      dst[0] = src[0];
      dst[1] = src[lda1];
      dst[2] = src[lda2];
      dst[3] = src[lda3];
  ...
  scalar_t *C, int ldc)
{
  ...
  const int aa_sz = (unsigned)(m_step_nom*k_step-1)/SIMD_FACTOR + 1;
